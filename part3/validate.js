 ...
 * 2. Sends each prompt in src/prompts.ts and captures the rewritten script.
 * 3. Grades each rewrite with an LLM judge against the rubric in src/rubric.ts.
...
import { printToConsole, score, summarise, writeReports } from './src/report.js';
...
async function main() {
  ...
  try {
    for (const [index, testPrompt] of TEST_PROMPTS.entries()) {
      ...
      try {
        const capture = await session.runPrompt(testPrompt.prompt);
        partial = {
          ...partial,
...
main().catch((error) => {
  ...
  console.error(
    '\nCommon causes:\n' +
      '  - No saved session: run `npm run auth` in part2/\n' +
      '  - ANTHROPIC_API_KEY not set in part3/.env\n' +
      '  - Selectors drifted: run `npm run discover` in part2/\n' +
      `  - Judge model unavailable: check JUDGE_MODEL (currently ${config.judgeModel})`,
  );
  process.exit(1);
});
